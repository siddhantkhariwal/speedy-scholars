#!/usr/bin/env python3
"""
Speedy Scholars — Intermediate B Book B Workbook Generator
Sequel to Intermediate B Book A. 47 pages.
"""
...
def page_multidigit_mult(c, pn):
    bg(c); y = elem_gen.draw_hdr_with_time(c, "Multiplication: 2-digit × 1-digit",
                                            "12 Mins",
                                            sub="Multi-digit multiplication")
    draw_footer(c, pn); scatter_decorations(c, pn)
    random.seed(pn + 10200)
    problems = [(random.randint(10, 99), random.randint(2, 9)) for _ in range(30)]
...
def page_cover(c):
    ...
    c.drawCentredString(W / 2, H - 115 * mm, "INTERMEDIATE B BOOK B")
    c.setFont("Helvetica", 14); c.setFillColor(GOLD)
    c.drawCentredString(W / 2, H - 130 * mm, "5th Level — Final Mastery Before Higher")
...
def main():
    ...
    for pn in range(23, 30):
        ...
        c.showPage()
    # p30-37: Division ÷2-÷9
    for pn, dvs in [(30, 2), (31, 3), (32, 4), (33, 5),
                     (34, 6), (35, 7), (36, 8), (37, 9)]:
    ...
    c.drawString(MARGIN, cur_y, "Mixed: Multiplication × Division")
    cur_y -= 16
    random.seed(46 + 10000)
    for i in range(20):
        if i % 2 == 0:
